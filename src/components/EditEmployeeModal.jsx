...
function EditEmployeeModal({ isOpen, onClose, onSubmit, editingEmployee, setEditingEmployee, settings }) {
  React.useEffect(() => {
    if (isOpen && settings && settings.poinTugasTambahan && editingEmployee) {
      const keys = Object.keys(settings.poinTugasTambahan);
      if (keys.length > 0 && !keys.includes(editingEmployee.tugasTambahan)) {
        setEditingEmployee(prev => prev ? ({ ...prev, tugasTambahan: keys[0] }) : null);
      }
    }
  }, [isOpen, settings, editingEmployee?.tugasTambahan, editingEmployee, setEditingEmployee]);
        ...
        <form onSubmit={onSubmit} className="p-6 space-y-4 overflow-y-auto flex-1">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5 col-span-1 md:col-span-2">
              ...
              <select
                value={editingEmployee.pendidikan || 'S1'}
